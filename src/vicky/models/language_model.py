"""
Módulo para interactuar con modelos de lenguaje
"""
import os
import logging
import torch
...
logger = logging.getLogger("vicky.models.language_model")

class LanguageModel:
    """
    Clase para interactuar con modelos de lenguaje como Qwen
    """
    ...
    def __init__(self, model_path: str = None, device: str = None, quantization: str = None):
        ...
        # Configurar modelo
        self.model_path = model_path or self.config["language_models"]["models"]["qwen"]["path"]
        self.device = device or self.config["language_models"]["models"]["qwen"]["device"]
        self.quantization = quantization or self.config["language_models"]["models"]["qwen"]["quantization"]
        
        self.model = None
        self.tokenizer = None
        
        logger.info(f"Inicializando modelo de lenguaje desde {self.model_path}")
    
    def load(self):
        """Carga el modelo en memoria."""
        try:
            logger.info(f"Cargando modelo de lenguaje desde {self.model_path}")
            ...
            logger.error(f"Error al cargar el modelo de lenguaje: {e}")
            return False
    ...
    def unload(self):
        ...
        logger.info("Modelo de lenguaje descargado de memoria")
